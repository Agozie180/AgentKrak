...
4. Run `python main.py stream --duration 30` to show live WebSocket ticks through Kraken CLI.
...
## Demo Narration
AgentKrak uses Kraken CLI subprocess calls for ticker, OHLC, order book, streaming ticker data, and paper trading. It computes RSI, EMA crossovers, MACD, Bollinger Bands, and bid/ask spread quality, then only emits BUY or SELL when three or more conditions agree. Every signal includes confidence and the exact conditions that fired, so the agent is explainable instead of a black box.
